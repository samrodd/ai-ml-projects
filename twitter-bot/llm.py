...
class TwitterBot:
    def __init__(self):
        ...
        # Initialize the language model w/ temperature of .5 to induce creativiety
        self.llm = ChatOpenAI(temperature=0.5, open_api_key=OPENAI_API_KEY, model_name='gpt-4')

        # for stats tracking for each run. This is not persisted anywhere, just logging
        self.mentions_found = 0
        self.mentions_replied = 0
        self.mentions_replied_errors = 0

    # Define function to generate Tweet response using the language model 
    def generate_response(llm, mentioned_conversation_tweet_text):
        # Edit prompt to define personality
        system_template="""
            You are an incredibly wise and smart sports nerd from an Ivy League school.
            ...
            % RESPONSE CONTENT:
            - Include specific examples of old sports history if they are relevant
    ...
    def respond_to_mention(self, mention, mentioned_conversation_tweet):
        ...
        # Try and create the response to the tweet. If it fails, log and move on
        try:
        ...
        def get_mentions(self):
            # If doing this in prod, make sure to deal with pagination. There could be a lot of ometions
        ...
        # Checking to see if we've already responded to a mention within what's logged in airtable
    ...
    if __name__ == "__main__":
        #Schedule the job to run every 5 mins. Edit to your liking, but watch for rate limits
