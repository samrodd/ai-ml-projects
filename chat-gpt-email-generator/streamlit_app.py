...
# Make changes to standard streamlit UI/UX
st.set_page_config(page_title="composify", page_icon="img/composify-logo-2.png")

# Design app further up and remove top padding
...
def gen_mail_contents(email_contents):
    ...
    for topic in range(len(email_contents)):
        ...
        # replace existing topic text with updated text
...
def main_gpt3emailgen():
    st.image('img/composify-banner.png')  # TITLE and Creator information
    st.markdown('Generate emails based on your directions - powered by AI (OpenAI GPT-3)')
    st.write('\n')  # add spacing
    st.subheader('\nWhat is your email about?\n')
    with st.expander("Section - Email Input", expanded=True):
        input_c1 = st.text_input('Enter email contents down below! Up to 2 separate topics supported', 'topic 1')
        input_c2 = st.text_input('', 'topic 2 (optional)')

        email_text = "" #initialize column variables
        c1, c2, c3, space, c4 = st.columns([5, 5, 5, 0.5, 5])
        with c1:
        ...
        with c4:
            ...
            if st.button('Generate Email'):
                with st.spinner():
                    ...
                    if(len(input_contents)==0): # remind user to provide inputs
                        st.write('Please fill in some contents for your message!')
                    if(len(input_contents)>=1): #initiate gpt3 e-mail generation
    ...
    if email_text != "":
        ...
        with st.expander("Section - Email Output", expanded=True):
            st.markdown(email_text) # output the result
